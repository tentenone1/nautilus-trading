...
class TestMalformedData:
    """6. malformed data behavior is documented."""

    def test_malformed_json_currently_raises_jsondecodeerror(self, monkeypatch, tmp_path):
        """
        RISKY CURRENT BEHAVIOR: _load_classifications() does not catch
        JSONDecodeError. A malformed JSON file causes get_category_action()
        to raise json.JSONDecodeError. There is no graceful fallback for
        bad JSON — callers (e.g. wf_signal_handler) must catch this or the
        entire signal pipeline crashes for that signal.
        """
        _clear_all_caches()
        ...
        with pytest.raises(json.JSONDecodeError):
            wfca.get_category_action("AnyWhale", "crypto")
...
class TestNullAction:
    """9. null action values in JSON propagate to result — risky but current."""

    def test_null_action_currently_returns_none(self, monkeypatch, tmp_path):
        """
        RISKY CURRENT BEHAVIOR: cat_entry.get("action", "INSUFFICIENT_DATA")
        does NOT handle None. The default only fires when the key is missing.
        If the key exists with value None (from a JSON with "action": null),
        None is returned directly as the action. This is a data integrity
        risk: downstream code that expects a string action will fail.
        """
        _clear_all_caches()
        json_path = _make_classifications_json({
            "WhaleJ": {
    ...
    def test_normal_fallback_returns_string_not_none(self, monkeypatch, tmp_path):
        """
        SAFE CURRENT BEHAVIOR: Under normal code paths (no nulls in JSON,
